...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 pb-24 relative">
      {/* Header */}
      ...
      <div className="text-center max-w-md w-full space-y-8">
        ...
        <div className="space-y-2">
          <h1 className="text-4xl font-bold text-white bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent">
            Cosmic Oracle
          </h1>
          <p className="text-white/70">Ask the universe your deepest questions</p>
        </div>
        {/* Magic Ball */}
        ...
        <div className="space-y-4">
          <textarea
            ...
            className="w-full p-4 rounded-xl bg-white/10 backdrop-blur-sm border border-white/20 text-white placeholder-white/50 resize-none focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent"
            rows={3}
          />
          <button
            onClick={handleAskQuestion}
            disabled={!question.trim() || isShaking || isTyping}
            className="w-full py-3 px-6 bg-gradient-to-r from-purple-500 to-cyan-500 text-white font-semibold rounded-xl hover:from-purple-600 hover:to-cyan-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-105"
          >
        ...
        {(answer || isTyping) && (
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20">
            ...
            {showAnswer && (
              <button
                onClick={handleShare}
                className="mt-4 flex items-center justify-center space-x-2 w-full py-2 px-4 bg-white/10 hover:bg-white/20 rounded-lg text-white/80 hover:text-white transition-colors"
              >
                <Share2 size={16} />
      ...
      {showShare && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 max-w-sm w-full border border-white/20">
            <h3 className="text-xl font-semibold text-white mb-4">Share Your Wisdom</h3>
